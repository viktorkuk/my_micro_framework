...
class DB {
    ...
    public function fetchRow($table, $id, $fields = array('*'), $object = false){
        $data = $this->rawQuery('SELECT '.implode(', ', $fields)." FROM $table WHERE id = ".intval($id),$object);
        if($data){
            return $data[0];
        }else{
            return false;
    ...
    public function updeteRow($table, $data, $id){
    ...
    public function delete($table, $id){
        return $this->rawQuery("DELETE FROM $table WHERE id = ".intval($id));
    }
